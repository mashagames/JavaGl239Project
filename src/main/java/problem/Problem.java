...
public void solve() {
    double sMax = 0;  // обнуляем максимальную площадь
    // перебираем пары углов
    for (int i=0;i<angles.size();i++) {
    for (int j=i+1;j<angles.size();j++){
        Angle a = angles.get(i);  // выбираем углы для работы в этом цикле
        Angle b = angles.get(j);
        if (a != b) { // если углы не совпадают
        // Инициализация переменных для пары углов
        Vector2 c1, c2, c3, c4, cMax, cMin;  // 4 точки пересечения, самая дальняя и самая ближняя из них
        double l1=0, l2=0, l3=0, l4=0, lMin=100, lMax=0; // расстояния до точек пересечения
        int beam11 = 0; int beam12 = 0; int beam21 = 0; int beam22 = 0; // счетчики точек на каждом луче
        c1 = cross (a.c, b.c, a.alpha1, b.alpha1); // поиск точек пересечения
        cMin = c1;  cMax = c1;
        if (c1.x!=-100) { // если найдено пересечение
            l1 = getlength(c1, a.c)+getlength(c1,b.c); // расстояние между углами через точку пересечения
            lMin = l1;  // определяем самое большое и самое маленькое расстояние
            lMax = l1;
            beam11 ++; // изменяем счетчики точек на лучах
            beam21 ++;
        }
        c2 = cross(a.c, b.c, a.alpha1, b.alpha2); // поиск точек пересечения
        if (c2.x!=-100) { // если найдено пересечение
            l2 = getlength(c2, a.c)+getlength(c2,b.c); // расстояние между углами через точку пересечения
            if(l2>lMax) {lMax=l2; cMax = c2; } // определяем самое большое и самое маленькое расстояние
            if(l2<lMin) {lMin=l2; cMin = c2; }
            beam11 ++; // изменяем счетчики точек на лучах
            beam22 ++;
        }
        c3 = cross(a.c, b.c, a.alpha2, b.alpha1); // поиск точек пересечения
        if (c3.x!=-100) { // если найдено пересечение
            l3 = getlength(c3, a.c)+getlength(c3,b.c); // расстояние между углами через точку пересечения
            if(l3>lMax) {lMax=l3; cMax = c3; } // определяем самое большое и самое маленькое расстояние
            if(l3<lMin) {lMin=l3; cMin = c3; }
            beam12++; // изменяем счетчики точек на лучах
            beam21++;
        }
        c4 = cross(a.c, b.c, a.alpha2, b.alpha2);
        if (c4.x!=-100) { // если найдено пересечение
            l4 = getlength(c4, a.c)+getlength(c4,b.c); // расстояние между углами через точку пересечения
            if(l4>lMax) { cMax = c4; } // определяем самое большое и самое маленькое расстояние
            if(l4<lMin) { cMin = c4; }
            beam12++; // изменяем счетчики точек на лучах
            beam22++;
        }
        ...
        if ((beam11 > 0) && (beam12 > 0) && (beam21 > 0) && (beam22 > 0) && (beam11+beam12+beam21+beam22)%4==0) {
            ...
            if (s > sMax) {
                sMax = s;   // запоминаем максимальную площадь
                a1sol = i;  // запоминаем номер первого угла-решения
                a2sol = j;  // запоминаем номер второго угла-решения
                maxA = a.c; // запоминаем вершины четырехугольника максимальной площади
                maxB = b.c;
                maxC = cMax;
                maxD = cMin;
            }
